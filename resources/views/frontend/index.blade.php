        ...
        <div class="hero__slider--inner hero__slider--activation swiper">
            <div class="hero__slider--wrapper swiper-wrapper">
                ...
                <div class="swiper-slide ">
                    <div class="hero__slider--items hero__slider--bg slider1" style="background: url({{ uploaded_asset($slider_images[$key]) }})">
                        <div class="container-fluid">
                            <div class="hero__slider--items__inner">
                                <div class="row row-cols-1">
                                    <div class=" {{ ($key %2 == 0) ? 'col' : 'col-lg-6 offset-lg-6'}}">
                                        <div class="slider__content">
                                            <!--<p class="slider__content--desc desc1 text-white mb-15">Discover our best furniture collection from home</p>--> 
                                            <h2 class="slider__content--maintitle text-white h1">{!! json_decode(get_setting('home_slider_titles'), true)[$key] !!}</h2>
                                            <p class="slider__content--desc text-white mb-35 d-sm-2-none">{!! json_decode(get_setting('home_slider_contents'), true)[$key] !!}</p>    
                                            <a class="slider__content--btn primary__btn" href="{{ json_decode(get_setting('home_slider_links'), true)[$key] }}">Start to Buying</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <section class="banner__section section--padding">
        <div class="container-fluid">
            ...
            <div class="row mb--n28">
                @php
                $digitInWord = ['one','two','three','four','five']; 
                $loopCnt = 0;
                @endphp
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 mb-28">
                    ...
                    <div class="banner__items {{ ($loopCnt >= 3) ? 'mb-25' : ''; }}">
                        <a class="banner__items--thumbnail position__relative" href="{{ route('products.category', $category->slug) }}">
                            <img class="banner__items--thumbnail__img" src="{{ uploaded_asset($category->banner) }}" alt="{{ $category->getTranslation('name') }}" />
                                <div class="banner__items--content {{ isset($digitInWord[$loopCnt]) ? $digitInWord[$loopCnt] : ''; }}">
                                    <h2 class="banner__items--content__title">{!! preg_replace('~((\w+\s){2})~', '$1' . "<br />\n", $category->getTranslation('name')) !!}</h2>
                                </div>
                        </a>
                    </div>
                        @if($loopCnt != 3)
    ...
    <div class="banner__video--section">
        <div class="container-fluid">
            <div class="banner__video--thumbnail position__relative">
                <img class="banner__video--thumbnail__img display-block" src="{{ static_asset('/assets/img/banner/banner-bg1.webp') }}" alt="banner-bideo-thumbnail">
...
<script>
    $(document).ready(function(){
        ...
        $.post('{{ route('home.section.best_sellers') }}', {_token:'{{ csrf_token() }}'}, function(data){
            ...
            $('body').on('click','#new-products-section li.product__tab--btn__list',function(){
                let $obj = $(this);
                let target = $obj.data('target');
                target = target.replace('#','');
                $('#new-products-section').find('li.product__tab--btn__list').removeClass('active');
                $('#new-products-section').find('.tab_content').find('.show').removeClass('show').removeClass('active');
                $obj.addClass('active');
                $('#new-products-section').find('.tab_content').find('#'+target).addClass('active').addClass('show');
            });
    });
</script>
@endsection
